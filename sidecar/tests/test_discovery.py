...
def test_probe_host_returns_candidate_when_ray_port_open(monkeypatch) -> None:
    monkeypatch.setattr("raylab_sidecar.discovery._tcp_open", lambda host, port, timeout: port == 6379)
...
def test_probe_host_includes_dashboard_when_reachable(monkeypatch) -> None:
    monkeypatch.setattr("raylab_sidecar.discovery._tcp_open", lambda host, port, timeout: True)
...
def test_probe_host_ports_finds_non_default_ray_port(monkeypatch) -> None:
    ...
    monkeypatch.setattr("raylab_sidecar.discovery._tcp_open", lambda host, port, timeout: port in open_ports)
...
def test_discovery_returns_reachable_configured_host_even_when_lan_scan_is_empty(monkeypatch) -> None:
    ...
    monkeypatch.setattr("raylab_sidecar.discovery._arp_neighbor_hosts", lambda: set())
    monkeypatch.setattr("raylab_sidecar.discovery.psutil.net_if_addrs", lambda: {})
    monkeypatch.setattr("raylab_sidecar.discovery._tcp_open", lambda host, port, timeout: host == "192.168.33.17" and port == 6379)
...
def test_discovery_surfaces_saved_host_even_when_probe_is_inconclusive(monkeypatch) -> None:
    ...
    monkeypatch.setattr("raylab_sidecar.discovery._arp_neighbor_hosts", lambda: set())
    monkeypatch.setattr("raylab_sidecar.discovery.psutil.net_if_addrs", lambda: {})
    monkeypatch.setattr("raylab_sidecar.discovery._tcp_open", lambda host, port, timeout: False)
